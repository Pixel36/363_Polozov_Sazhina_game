...
// Settings represents the saved game settings
...
const (
	// Экран
	screenW  = 1920 // ширина окна
	screenH  = 1080 // высота окна
	tileSize = 32   // размер тайла карты в пикселях

	// Чат
	chatHeightFixed = 400 // высота области чата

	// Таймер хода
	turnTimeout = 20.0 // длительность хода в секундах

	// Геометрия оружия
	swordHiltLen    = 16.0 // длина рукояти меча
	swordHiltW      = 6.0  // ширина рукояти
	swordCrossLen   = 24.0 // длина гарды
	swordCrossW     = 4.0  // ширина гарды
	swordBladeLen   = 35.0 // длина клинка
	swordBladeBaseW = 14.0 // ширина у основания клинка
	swordBladeTipW  = 5.0  // ширина у острия

	spearHeadLen  = 30.0 // длина наконечника копья
	spearHeadW    = 12.0 // ширина наконечника
	spearShaftLen = 50.0 // длина древка
	spearShaftW   = 5.0  // ширина древка
	spearButtLen  = 10.0 // длина подтока
	spearButtW    = 8.0  // ширина подтока

	// Анимация удара
	moveDuration       = 0.05 // длительность перемещения (плавное движение)
	attackAnimDuration = 0.2  // длительность анимации удара (сек)
)

// ==================== СТРУКТУРЫ ====================

// NetColor – цвет в формате, понятном серверу (RGBA)
...
// Player – данные игрока (клиентская копия)
type Player struct {
	ID          string  // уникальный идентификатор
	Name        string  // имя игрока
	Race        string  // раса ("human" или "cat")
	Weapon      string  // оружие ("sword" / "spear")
	X, Y        float64 // текущие координаты (интерполированные)
	TargetX     float64 // целевые координаты (от сервера)
	TargetY     float64
	HP          int           // здоровье
	Color       NetColor      // цвет игрока
	Image       *ebiten.Image // кэшированное изображение цветного квадрата
	LastUpdate  time.Time     // время последнего обновления от сервера
	Initialized bool          // флаг инициализации
	IsMe        bool          // true, если это текущий игрок

	// Поля для плавного движения
	Moving        bool      // движется ли сейчас
	MoveStartTime time.Time // время начала движения
	MoveStartX    float64   // начальная позиция X
	MoveStartY    float64   // начальная позиция Y
	MoveEndX      float64   // конечная позиция X
	MoveEndY      float64   // конечная позиция Y

	// Поля для анимации удара
	AttackAnimStart    time.Time // время начала анимации
	AttackAnimTargetID string    // ID цели
	AttackAnimType     string    // тип оружия ("sword" / "spear")
	AttackAnimProgress float64   // прогресс 0..1
}
...
type ChatMessage struct {
	...
	Time  int64    // временная метка (мс)
	Color NetColor // цвет отправителя
}

// MainMenuButton – структура кнопки главного меню
type MainMenuButton struct {
	Text   string        // надпись
	Action func(g *Game) // действие при нажатии
}

// Game – главная структура игры, реализует интерфейс ebiten.Game
type Game struct {
	// Состояние приложения
	state string // "mainmenu", "character", "game", "settings"

	// Состояние перехода между экранами
	previousState string // предыдущее состояние для корректного возврата
	...
	// Анимация оружия – только для текущего игрока
	...
	lastFullscreenToggle time.Time
	...
	currentMusic string // "menu", "game" или "none"
}

// ==================== ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ====================

// createGlowImage создаёт изображение красной рамки для подсветки врага
...
// createPlayerImage создаёт квадрат заданного цвета для отображения игрока
...
// generateMenuColors генерирует палитру из 20 равномерно распределённых цветов
...
// hsvToRgb преобразует HSV в RGB (возвращает значения 0..1)
...
// generateRock – рекурсивная генерация камня (для фона меню)
...
// generateMainMenuMap генерирует случайную карту для фона главного меню
...
// splitLongWord разбивает длинное слово на части, помещающиеся в maxWidth
...
// wrapText переносит текст по словам с учётом максимальной ширины
...
// initTileCache создаёт кэш изображений тайлов с текстурой (лёгкий градиент)
...
// loadMusic загружает и декодирует Ogg Vorbis из файла, возвращая плеер с бесконечным циклом
func (g *Game) loadMusic(filename string) (*audio.Player, error) {
	...
	// Декодируем Ogg Vorbis, передавая аудиоконтекст
	stream, err := vorbis.Decode(g.audioContext, bytes.NewReader(data))
	if err != nil {
		return nil, err
	}
	...
	// Создаём плеер
	return audio.NewPlayer(g.audioContext, loop)
}

// stopMusic останавливает всю играющую музыку
func (g *Game) stopMusic() {
	if g.menuMusic != nil && g.menuMusic.IsPlaying() {
		g.menuMusic.Pause()
	}
	if g.gameMusic != nil && g.gameMusic.IsPlaying() {
		g.gameMusic.Pause()
	}
}

// updateMusicVolume обновляет громкость всех музыкальных плееров
func (g *Game) updateMusicVolume() {
	vol := float64(g.volume) / 100.0
	if g.menuMusic != nil {
		g.menuMusic.SetVolume(vol)
	}
	if g.gameMusic != nil {
		g.gameMusic.SetVolume(vol)
	}
}

// readLoop – горутина чтения сообщений от сервера
func (g *Game) readLoop() {
	...
	for {
		...
		if err != nil {
			// Если соединение закрыто, не выводим ошибку в лог (это ожидаемо)
...
// handleInit обрабатывает сообщение "init" от сервера
func (g *Game) handleInit(msg map[string]interface{}) {
	...
	if id, ok := msg["id"].(string); ok {
		...
		log.Printf("Инициализирован с ID: %s, позиция: %.0f,%.0f, раса: %s, оружие: %s", id, startX, startY, race, g.charWeapon)
	}
}

// handleMap обрабатывает сообщение "map" от сервера
...
// handleState обрабатывает сообщение "state" от сервера (список игроков)
	...
	if data, ok := msg["data"].([]interface{}); ok {
		...
		for _, pData := range data {
			if playerMap, ok := pData.(map[string]interface{}); ok {
				...
				weapon, _ := playerMap["weapon"].(string)
...
// handleChatMessage обрабатывает входящее сообщение чата
...
func (g *Game) Update() error {
	...
	// Переключение музыки в зависимости от состояния
	desired := "menu"
	if g.state == "game" && g.connected && g.ready {
		desired = "game"
	} else if g.state == "settings" {
		// В настройках музыка зависит от предыдущего состояния
		if g.previousState == "game" {
			desired = "game"
		} else {
			desired = "menu"
		}
...
// handleDeathScreen обрабатывает экран смерти
...
// handleQuitConfirm обрабатывает диалог подтверждения выхода
func (g *Game) handleQuitConfirm() {
	...
	// Первый ряд: Нет и Выйти
	btnY1 := dy + 150
	startX1 := dx + (dw-2*btnW-spacing)/2

	noRect := image.Rect(startX1, btnY1, startX1+btnW, btnY1+btnH)
	exitRect := image.Rect(startX1+btnW+spacing, btnY1, startX1+2*btnW+spacing, btnY1+btnH)

	// Второй ряд: Настройки и Главное меню
	btnY2 := btnY1 + btnH + spacing
	settingsRect := image.Rect(startX1, btnY2, startX1+btnW, btnY2+btnH)
	menuRect := image.Rect(startX1+btnW+spacing, btnY2, startX1+2*btnW+spacing, btnY2+btnH)

	// Сохраняем прямоугольники для отрисовки
	g.quitConfirmRects.yes = menuRect
	g.quitConfirmRects.no = noRect
	g.quitConfirmRects.exit = exitRect
	// Для кнопки настроек нет отдельного поля, но она будет обработана здесь

	if ebiten.IsMouseButtonPressed(ebiten.MouseButtonLeft) {
		x, y := ebiten.CursorPosition()
		pt := image.Pt(x, y)

		if pt.In(settingsRect) {
			g.previousState = g.state
			g.state = "settings"
			g.showQuitConfirm = false
		} else if pt.In(menuRect) {
			...
			g.state = "mainmenu"
			g.showQuitConfirm = false
			// Принудительно сбрасываем музыку, чтобы при входе в меню она перезапустилась
			g.stopMusic()
			g.currentMusic = "none"
		} else if pt.In(noRect) {
			g.showQuitConfirm = false
		} else if pt.In(exitRect) {
...
// updateMainMenu обновляет логику главного меню
...
// updateSettings обновляет логику меню настроек
func (g *Game) updateSettings() {
	sliderX, sliderY := 300, 250
	...
	if ebiten.IsMouseButtonPressed(ebiten.MouseButtonLeft) {
		...
		if pt.In(g.fullscreenBtn) {
			now := time.Now()
			if now.Sub(g.lastFullscreenToggle) > 200*time.Millisecond {
				g.fullscreen = !g.fullscreen
				ebiten.SetFullscreen(g.fullscreen)
				g.lastFullscreenToggle = now
			}
		}
	...
	// Handle keyboard volume changes with proper state management
...
// updateCharacterMenu обновляет логику меню создания персонажа
...
// fetchUsedColors запрашивает с сервера список занятых цветов
...
// updatePreview обновляет изображение предпросмотра персонажа
...
// updateGame обновляет логику игрового процесса
func (g *Game) updateGame() error {
	...
	if myTurn && leftPressed && !g.prevLeftMouse {
		x, y := ebiten.CursorPosition()
		...
		if g.myPlayer != nil {
			...
			if targetPlayer != nil {
				...
				if dx+dy <= float64(attackRange) && !(dx == 0 && dy == 0) {
					...
					g.mu.Lock()
					if g.myPlayer != nil {
						g.myPlayer.AttackAnimStart = time.Now()
						g.myPlayer.AttackAnimTargetID = targetPlayer.ID
						g.myPlayer.AttackAnimType = g.charWeapon
						g.myPlayer.AttackAnimProgress = 0.0
					}
					g.mu.Unlock()
				}
			} else {
	...
	for _, pl := range g.players {
		if pl.Initialized {
			...
			if !pl.AttackAnimStart.IsZero() {
				elapsed := now.Sub(pl.AttackAnimStart).Seconds()
				if elapsed >= attackAnimDuration {
					pl.AttackAnimStart = time.Time{}
					pl.AttackAnimTargetID = ""
					pl.AttackAnimProgress = 0.0
				} else {
					pl.AttackAnimProgress = elapsed / attackAnimDuration
				}
			}
		}
	}
	...
	g.prevLeftMouse = leftPressed

	return nil
}

// handleChatInput обрабатывает ввод в чате
...
// Draw отрисовывает всё на экране
...
// drawDeathScreen отрисовывает экран смерти
...
// drawCharacterMenu отрисовывает меню создания персонажа
func (g *Game) drawCharacterMenu(screen *ebiten.Image) {
	screen.Fill(color.RGBA{0xe5, 0xdb, 0xb8, 0xff})

	// Создаем увеличенный шрифт для элементов редактора персонажа
	var charEditorFont font.Face
	if g.logoFontFace != nil {
		// Используем логотипный шрифт как основу для увеличенного шрифта
		ttfData, err := os.ReadFile("medieval.ttf")
		if err == nil {
			tt, err := opentype.Parse(ttfData)
			if err == nil {
				charEditorFont, err = opentype.NewFace(tt, &opentype.FaceOptions{
					Size:    40,
					DPI:     72,
	...
	if charEditorFont == nil {
		// Резервный шрифт увеличенного размера
	...
	if charEditorFont == nil {
		charEditorFont = g.fontFace
	}

	title := "Создание персонажа"
	bounds := text.BoundString(charEditorFont, title)
	text.Draw(screen, title, charEditorFont, (screenW-bounds.Dx())/2, 100, color.Black)

	nameLabel := "Имя:"
	text.Draw(screen, nameLabel, charEditorFont, 200, 180, color.Black)
	inputRect := image.Rect(400, 140, 900, 240) // Увеличили высоту
	...
	text.Draw(screen, displayName, charEditorFont, int(float64(inputRect.Min.X))+10, int(float64(inputRect.Min.Y))+70, color.Black) // Скорректировали позицию текста

	raceLabel := "Раса:"
	text.Draw(screen, raceLabel, charEditorFont, 200, 320, color.Black) // Скорректировали позицию

	humanBtn := image.Rect(400, 280, 650, 380) // Увеличили кнопки
	...
	humanText := "Человек"
	boundsHuman := text.BoundString(charEditorFont, humanText)
	txHuman := humanBtn.Min.X + (humanBtn.Dx()-boundsHuman.Dx())/2
	tyHuman := humanBtn.Min.Y + (humanBtn.Dy()+boundsHuman.Dy())/2
	text.Draw(screen, humanText, charEditorFont, txHuman, tyHuman, color.Black)

	catBtn := image.Rect(670, 280, 920, 380) // Увеличили кнопки
	...
	catText := "Кот"
	boundsCat := text.BoundString(charEditorFont, catText)
	txCat := catBtn.Min.X + (catBtn.Dx()-boundsCat.Dx())/2
	tyCat := catBtn.Min.Y + (catBtn.Dy()+boundsCat.Dy())/2
	text.Draw(screen, catText, charEditorFont, txCat, tyCat, color.Black)

	weaponLabel := "Оружие:"
	text.Draw(screen, weaponLabel, charEditorFont, 200, 440, color.Black) // Скорректировали позицию

	swordBtn := image.Rect(400, 400, 650, 500) // Увеличили кнопки
	...
	g.drawSwordScaled(screen, swordCenterX, swordCenterY, 0, 2.0, nil) // Увеличили масштаб оружия

	spearBtn := image.Rect(670, 400, 920, 500) // Увеличили кнопки
	...
	g.drawSpearScaled(screen, spearCenterX, spearCenterY, 0, 2.0, nil) // Увеличили масштаб оружия

	colorLabel := "Цвет:"
	text.Draw(screen, colorLabel, charEditorFont, 200, 640, color.Black) // Сдвинули позицию цвета ниже

	startX, startY := 400, 600 // Сдвинули позицию цветовых кнопок ниже
	sw, sh := 80, 80           // Увеличили размеры цветовых кнопок
	spacing := 20              // Увеличили расстояние между кнопками
	for i, c := range g.charColors {
		...
		if i == g.charSelectedColor {
			ebitenutil.DrawRect(screen, float64(x-3), float64(y-3), float64(sw+6), float64(sh+6), color.White) // Увеличили рамку выделения
		}
		if g.colorTaken[i] {
	...
	if g.charPreviewImg != nil {
		op := &ebiten.DrawImageOptions{}
		op.GeoM.Scale(5, 5) // Увеличили масштаб предпросмотра
		op.GeoM.Translate(1000, 300)
		screen.DrawImage(g.charPreviewImg, op)

		centerX := 1000 + float64(tileSize)*2.5 // Скорректировали центр
		centerY := 300 + float64(tileSize)*2.5  // Скорректировали центр

		if g.charRace == "cat" && g.charSelectedColor >= 0 {
			col := g.charColors[g.charSelectedColor]
			netCol := NetColor{R: col.R, G: col.G, B: col.B, A: col.A}
			g.drawCatEarsScaled(screen, centerX, centerY, netCol, 5.0) // Увеличили масштаб ушей
		}

		// Рисуем оружие в руках персонажа
		if g.charWeapon == "sword" {
			// Позиция для меча в правой руке персонажа (с учетом смещения для правильного расположения в руке)
			weaponX := centerX + 80 // Сдвинули оружие левее, не меняя положение персонажа
			weaponY := centerY + 20
			g.drawSwordScaled(screen, weaponX, weaponY, math.Pi/4, 2.0, nil) // Увеличили масштаб и угол для лучшего визуала
		} else {
			// Позиция для копья в правой руке персонажа (с учетом смещения для правильного расположения в руке)
			weaponX := centerX + 80 // Сдвинули оружие левее, не меняя положение персонажа
			weaponY := centerY + 20
			g.drawSpearScaled(screen, weaponX, weaponY, math.Pi/4, 2.0, nil) // Увеличили масштаб и угол для лучшего визуала
		}
	}

	connectBtn := image.Rect(screenW/2+100, 800, screenW/2+500, 900) // Сдвинули кнопку "Подключиться" правее
	...
	boundsConn := text.BoundString(charEditorFont, connectText)
	txConn := connectBtn.Min.X + (connectBtn.Dx()-boundsConn.Dx())/2
	tyConn := connectBtn.Min.Y + (connectBtn.Dy()+boundsConn.Dy())/2
	text.Draw(screen, connectText, charEditorFont, txConn, tyConn, color.Black)

	if g.charError != "" {
		text.Draw(screen, "Ошибка: "+g.charError, charEditorFont, 200, 950, color.RGBA{255, 0, 0, 255}) // Скорректировали позицию
	}
}

// drawSwordScaled рисует меч с заданным масштабом
func (g *Game) drawSwordScaled(screen *ebiten.Image, x, y, angle, scale float64, player *Player) {
	progress := 0.0
	if player != nil && !player.AttackAnimStart.IsZero() && player.AttackAnimType == "sword" {
		progress = player.AttackAnimProgress
		swingAngle := (progress - 0.5) * math.Pi / 2
		angle += swingAngle
		x += math.Cos(angle) * 10 * progress
		y += math.Sin(angle) * 10 * progress
	}
	...
	crossImg.Fill(color.RGBA{80, 80, 255, 255})
...
// drawSpearScaled рисует копьё с заданным масштабом
func (g *Game) drawSpearScaled(screen *ebiten.Image, x, y, angle, scale float64, player *Player) {
	offsetX, offsetY := 0.0, 0.0
	if player != nil && !player.AttackAnimStart.IsZero() && player.AttackAnimType == "spear" {
		progress := player.AttackAnimProgress
		poke := math.Sin(progress*math.Pi) * 30
		offsetX = math.Cos(angle) * poke
		offsetY = math.Sin(angle) * poke
	}
	x += offsetX
	y += offsetY
...
// drawSettings отрисовывает меню настроек
func (g *Game) drawSettings(screen *ebiten.Image) {
	...
	boundsFull := text.BoundString(g.fontFace, fullText)
	...
	text.Draw(screen, fullText, g.fontFace, txFull, tyFull, color.Black)
...
// drawCatEarsScaled рисует кошачьи уши
...
// drawGame отрисовывает игровой экран
func (g *Game) drawGame(screen *ebiten.Image) {
	...
	for _, pl := range playersCopy {
		...
		switch pl.Weapon {
		case "sword":
			g.drawSwordScaled(screen, pl.X-camX, pl.Y-camY, math.Pi/4, 1.0, pl)
		case "spear":
			g.drawSpearScaled(screen, pl.X-camX, pl.Y-camY, math.Pi/4, 1.0, pl)
		}
	}

	if meCopy != nil {
		if g.charWeapon == "sword" {
			g.drawSwordScaled(screen, meCopy.X-camX, meCopy.Y-camY, g.mySwordCurrentAngle, 1.0, meCopy)
		} else {
			g.drawSpearScaled(screen, meCopy.X-camX, meCopy.Y-camY, g.mySwordCurrentAngle, 1.0, meCopy)
		}
	}
...
// drawTurnTimer отрисовывает индикатор хода и таймер
...
// fillTriangle заполняет треугольник заданным цветом
...
func (g *Game) drawChat(screen *ebiten.Image, chatHistory []ChatMessage, chatOpen bool, chatBuffer string, chatCursor bool, _ time.Time, chatCursorTimer time.Time) {
	...
	for i := startIdx; i < endIdx; i++ {
		...
		if line.nick != "" {
			text.Draw(screen, line.nick, g.chatFontFace, int(float64(xPos)), int(float64(yPos)), line.nickColor)
			xPos += int(text.BoundString(g.chatFontFace, line.nick).Dx())
		}
		text.Draw(screen, line.text, g.chatFontFace, int(float64(xPos)), int(float64(yPos)), color.White)
		yPos += lineHeight
	}
	...
	if chatOpen {
		...
		inputBg.Fill(color.RGBA{40, 40, 220, 220})
		...
		for _, line := range inputLines {
			text.Draw(screen, line, g.chatFontFace, int(float64(inputX+5)), int(float64(yInput+lineHeight)), color.White)
			yInput += lineHeight
		}

		if chatCursor && time.Since(chatCursorTimer) < 500*time.Millisecond {
			lastLine := inputLines[len(inputLines)-1]
			cursorX := inputX + 5 + int(text.BoundString(g.chatFontFace, lastLine).Dx())
			cursorY := yInput - lineHeight + 5
			text.Draw(screen, "_", g.chatFontFace, int(float64(cursorX)), int(float64(cursorY)), color.White)
		}
	}
}

// drawQuitConfirm отрисовывает диалог подтверждения выхода
func (g *Game) drawQuitConfirm(screen *ebiten.Image) {
	...
	btnW, btnH := 220, 50
	spacing := 20
	btnY1 := g.quitConfirmRects.bg.Min.Y + 150
	startX1 := g.quitConfirmRects.bg.Min.X + (g.quitConfirmRects.bg.Dx()-2*btnW-spacing)/2
	...
	noImg := ebiten.NewImage(btnW, btnH)
	noImg.Fill(btnColor)
	opNo := &ebiten.DrawImageOptions{}
	opNo.GeoM.Translate(float64(noRect.Min.X), float64(noRect.Min.Y))
	screen.DrawImage(noImg, opNo)
	noText := "Нет"
	noBounds := text.BoundString(g.fontFace, noText)
	noX := noRect.Min.X + (btnW-noBounds.Dx())/2
	noY := noRect.Min.Y + (btnH+noBounds.Dy())/2
	text.Draw(screen, noText, g.fontFace, noX, noY, color.Black)

	// Отрисовка кнопки "Выйти"
	exitImg := ebiten.NewImage(btnW, btnH)
	exitImg.Fill(exitBtnColor)
	opExit := &ebiten.DrawImageOptions{}
	opExit.GeoM.Translate(float64(exitRect.Min.X), float64(exitRect.Min.Y))
	screen.DrawImage(exitImg, opExit)
	exitText := "Выйти"
	exitBounds := text.BoundString(g.fontFace, exitText)
	exitX := exitRect.Min.X + (btnW-exitBounds.Dx())/2
	exitY := exitRect.Min.Y + (btnH+exitBounds.Dy())/2
	text.Draw(screen, exitText, g.fontFace, exitX, exitY, color.Black)

	// Отрисовка кнопки "Настройки"
	settingsImg := ebiten.NewImage(btnW, btnH)
	settingsImg.Fill(btnColor)
	opSettings := &ebiten.DrawImageOptions{}
	opSettings.GeoM.Translate(float64(settingsRect.Min.X), float64(settingsRect.Min.Y))
	screen.DrawImage(settingsImg, opSettings)
	settingsText := "Настройки"
	settingsBounds := text.BoundString(g.fontFace, settingsText)
	settingsX := settingsRect.Min.X + (btnW-settingsBounds.Dx())/2
	settingsY := settingsRect.Min.Y + (btnH+settingsBounds.Dy())/2
	text.Draw(screen, settingsText, g.fontFace, settingsX, settingsY, color.Black)

	// Отрисовка кнопки "Главное меню"
	menuImg := ebiten.NewImage(btnW, btnH)
	menuImg.Fill(btnColor)
	opMenu := &ebiten.DrawImageOptions{}
	opMenu.GeoM.Translate(float64(menuRect.Min.X), float64(menuRect.Min.Y))
	screen.DrawImage(menuImg, opMenu)
	menuText := "Главное меню"
	menuBounds := text.BoundString(g.fontFace, menuText)
	menuX := menuRect.Min.X + (btnW-menuBounds.Dx())/2
	menuY := menuRect.Min.Y + (btnH+menuBounds.Dy())/2
	text.Draw(screen, menuText, g.fontFace, menuX, menuY, color.Black)
}
...
// disconnect закрывает соединение и сбрасывает состояние
...
func loadSettings() *Settings {
	...
	if err != nil {
		fmt.Println("Не удалось открыть файл настроек, используются значения по умолчанию:", err)
		return settings
	}
	defer file.Close()
	...
	if err != nil {
		fmt.Println("Ошибка при чтении настроек, используются значения по умолчанию:", err)
		return &Settings{
			Volume:     50,
			Fullscreen: true,
		}
...
func main() {
	...
	if player, err := game.loadMusic("menu.ogg"); err == nil {
		game.menuMusic = player
	} else {
		log.Println("Не удалось загрузить menu.ogg:", err)
	}
	if player, err := game.loadMusic("game.ogg"); err == nil {
		game.gameMusic = player
	} else {
		log.Println("Не удалось загрузить game.ogg:", err)
	}
