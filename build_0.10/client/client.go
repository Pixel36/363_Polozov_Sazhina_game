...
func (g *Game) Update() error {
	...
	desired := "menu" // во всех неигровых состояниях играет музыка меню
...
func (g *Game) handleQuitConfirm() {
	...
	// Calculate rectangles for four buttons: Settings, Main Menu, No, Exit
	btnW, btnH := 160, 50
	spacing := 10
	totalBtnWidth := 4*btnW + 3*spacing
	startX := dx + (dw-totalBtnWidth)/2
	btnY := dy + 180

	// Settings button
	settingsRect := image.Rect(startX, btnY, startX+btnW, btnY+btnH)
	// Main Menu button
	yesRect := image.Rect(startX+btnW+spacing, btnY, startX+2*btnW+spacing, btnY+btnH)
	// No button
	noRect := image.Rect(startX+2*(btnW+spacing), btnY, startX+3*btnW+2*spacing, btnY+btnH)
	// Exit button
	exitRect := image.Rect(startX+3*(btnW+spacing), btnY, startX+4*btnW+3*spacing, btnY+btnH)
...
func (g *Game) updateSettings() {
	...
	if ebiten.IsKeyPressed(ebiten.KeyLeft) {
		...
		if oldVolume != g.volume { // Only update if volume actually changed
			g.updateMusicVolume()
		}
	}
...
func (g *Game) drawQuitConfirm(screen *ebiten.Image) {
	...
	// Calculate positions for four buttons: Settings, Main Menu, No, Exit
	btnW, btnH := 160, 50
	spacing := 10
	totalWidth := 4*btnW + 3*spacing
	startX := g.quitConfirmRects.bg.Min.X + (g.quitConfirmRects.bg.Dx()-totalWidth)/2
	btnY := g.quitConfirmRects.bg.Min.Y + 180

	// Settings button
	settingsImg := ebiten.NewImage(btnW, btnH)
	settingsImg.Fill(btnColor)
	opSettings := &ebiten.DrawImageOptions{}
	opSettings.GeoM.Translate(float64(startX), float64(btnY))
	screen.DrawImage(settingsImg, opSettings)
	settingsText := "Настройки"
	settingsBounds := text.BoundString(g.fontFace, settingsText)
	settingsX := startX + (btnW-settingsBounds.Dx())/2
	settingsY := btnY + (btnH+settingsBounds.Dy())/2
	text.Draw(screen, settingsText, g.fontFace, settingsX, settingsY, color.Black)

	// Yes/Main Menu button
	yesImg := ebiten.NewImage(btnW, btnH)
	yesImg.Fill(btnColor)
	opYes := &ebiten.DrawImageOptions{}
	opYes.GeoM.Translate(float64(startX+btnW+spacing), float64(btnY))
	screen.DrawImage(yesImg, opYes)
	yesText := "Главное меню"
	yesBounds := text.BoundString(g.fontFace, yesText)
	yesX := startX + btnW + spacing + (btnW-yesBounds.Dx())/2
	yesY := btnY + (btnH+yesBounds.Dy())/2
	...
	opNo.GeoM.Translate(float64(startX+2*(btnW+spacing)), float64(btnY))
	screen.DrawImage(noImg, opNo)
	noText := "Нет"
	noBounds := text.BoundString(g.fontFace, noText)
	noX := startX + 2*(btnW+spacing) + (btnW-noBounds.Dx())/2
	noY := btnY + (btnH+noBounds.Dy())/2
	...
	opExit.GeoM.Translate(float64(startX+3*(btnW+spacing)), float64(btnY))
	screen.DrawImage(exitImg, opExit)
	exitText := "Выйти"
	exitBounds := text.BoundString(g.fontFace, exitText)
	exitX := startX + 3*(btnW+spacing) + (btnW-exitBounds.Dx())/2
	exitY := btnY + (btnH+exitBounds.Dy())/2
