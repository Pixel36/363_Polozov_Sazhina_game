...
type Player struct {
	...
	// Поля для интерполяции движения
	Moving        bool
...
func main() {
	fmt.Println("=== Мультиплеерная RPG (пошаговая) ===")
		...
		for _, pData := range data {
			if playerMap, ok := pData.(map[string]interface{}); ok {
				...
				if !exists {
					...
					// Обновляем TargetX/TargetY и запускаем интерполяцию при изменении
	...
	for _, pl := range g.players {
		if pl.Initialized {
			if pl.Moving {
				...
				// Если не движется, но координаты отличаются от целевых (например, при телепортации)
...
func (g *Game) drawGame(screen *ebiten.Image) {
				...
				if tileX >= 0 && tileX < len(gameMapCopy[0]) && tileY >= 0 && tileY < len(gameMapCopy) {
					if gameMapCopy[tileY][tileX] == 0 {
						...
						if !occupied {
							...
							highlight.Fill(color.RGBA{0, 180, 0, 70})
							op := &ebiten.DrawImageOptions{}
	...
	for _, pl := range playersCopy {
		...
		if hoveredEnemyID == pl.ID && myTurn {
...
// drawSword рисует меч с правильным расположением частей: рукоять, гарда, лезвие.
func (g *Game) drawSword(screen *ebiten.Image, x, y float64, angle float64) {
	// Рукоять: центр на половине длины от руки
	hiltCenterX := x + (swordHiltLen/2)*math.Cos(angle)
	hiltCenterY := y + (swordHiltLen/2)*math.Sin(angle)
	hiltImg := ebiten.NewImage(int(swordHiltLen), int(swordHiltW))
	hiltImg.Fill(color.RGBA{139, 69, 19, 255}) // коричневая
	opHilt := &ebiten.DrawImageOptions{}
	opHilt.GeoM.Translate(-swordHiltLen/2, -swordHiltW/2)
	opHilt.GeoM.Rotate(angle)
	opHilt.GeoM.Translate(hiltCenterX, hiltCenterY)
	screen.DrawImage(hiltImg, opHilt)

	// Гарда: в конце рукояти
	guardX := x + swordHiltLen*math.Cos(angle)
	guardY := y + swordHiltLen*math.Sin(angle)
	crossImg := ebiten.NewImage(int(swordCrossLen), int(swordCrossW))
	crossImg.Fill(color.RGBA{80, 80, 80, 255}) // серый металл
	opCross := &ebiten.DrawImageOptions{}
	opCross.GeoM.Translate(-swordCrossLen/2, -swordCrossW/2)
	opCross.GeoM.Rotate(angle + math.Pi/2)
	opCross.GeoM.Translate(guardX, guardY)
	screen.DrawImage(crossImg, opCross)

	// Лезвие: от гарды до острия
	bladeStartX := guardX
	bladeStartY := guardY
	bladeEndX := guardX + swordBladeLen*math.Cos(angle)
	bladeEndY := guardY + swordBladeLen*math.Sin(angle)

	perpX := -math.Sin(angle)
	perpY := math.Cos(angle)

	// Основание лезвия у гарды
	baseLeftX := bladeStartX + (swordBladeBaseW/2)*perpX
	baseLeftY := bladeStartY + (swordBladeBaseW/2)*perpY
	baseRightX := bladeStartX - (swordBladeBaseW/2)*perpX
	baseRightY := bladeStartY - (swordBladeBaseW/2)*perpY

	// Остриё
	tipLeftX := bladeEndX + (swordBladeTipW/2)*perpX
	tipLeftY := bladeEndY + (swordBladeTipW/2)*perpY
	tipRightX := bladeEndX - (swordBladeTipW/2)*perpX
	tipRightY := bladeEndY - (swordBladeTipW/2)*perpY
...
// drawTurnTimer рисует песочные часы с корректным заполнением.
func (g *Game) drawTurnTimer(screen *ebiten.Image, timeLeft float64, myTurn bool, currentPlayerName string) {
	...
	timeStr := fmt.Sprintf("%.1f с", timeLeft)
	...
	// Контуры треугольников (не менять)
	vector.StrokeLine(screen, float32(cx), float32(waistY), float32(cx-baseW/2), float32(topY), 2, color.Gray{Y: 100}, false)
	vector.StrokeLine(screen, float32(cx), float32(waistY), float32(cx+baseW/2), float32(topY), 2, color.Gray{Y: 100}, false)
	vector.StrokeLine(screen, float32(cx-baseW/2), float32(topY), float32(cx+baseW/2), float32(topY), 2, color.Gray{Y: 100}, false)

	vector.StrokeLine(screen, float32(cx), float32(waistY), float32(cx-baseW/2), float32(bottomY), 2, color.Gray{Y: 100}, false)
	vector.StrokeLine(screen, float32(cx), float32(waistY), float32(cx+baseW/2), float32(bottomY), 2, color.Gray{Y: 100}, false)
	vector.StrokeLine(screen, float32(cx-baseW/2), float32(bottomY), float32(cx+baseW/2), float32(bottomY), 2, color.Gray{Y: 100}, false)
	...
	// Верхний треугольник (перевёрнутый) — заполняется сверху вниз, количество песка = fillRatio
	if fillRatio > 0 {
		fillYTop := waistY - fillRatio*(height/2)
		if fillYTop < topY {
			fillYTop = topY
		}
		// ширина линейно изменяется от 0 у вершины до baseW у основания
		widthAtY := baseW * (waistY - fillYTop) / (height / 2)
		leftX := cx - widthAtY/2
		rightX := cx + widthAtY/2

		vertices := []ebiten.Vertex{
			{DstX: float32(cx), DstY: float32(waistY), SrcX: 0, SrcY: 0, ColorR: 210.0 / 255, ColorG: 180.0 / 255, ColorB: 140.0 / 255, ColorA: 200.0 / 255},
			{DstX: float32(leftX), DstY: float32(fillYTop), SrcX: 0, SrcY: 0, ColorR: 210.0 / 255, ColorG: 180.0 / 255, ColorB: 140.0 / 255, ColorA: 200.0 / 255},
			{DstX: float32(rightX), DstY: float32(fillYTop), SrcX: 0, SrcY: 0, ColorR: 210.0 / 255, ColorG: 180.0 / 255, ColorB: 140.0 / 255, ColorA: 200.0 / 255},
		}
		indices := []uint16{0, 1, 2}
		screen.DrawTriangles(vertices, indices, whiteTex, nil)
	}

	// Нижний треугольник (обычный) — заполняется снизу вверх, количество песка = 1 - fillRatio
	if fillRatio < 1 {
		fillYBottom := waistY + (1-fillRatio)*(height/2)
		if fillYBottom > bottomY {
			fillYBottom = bottomY
		}
		widthAtY := baseW * (fillYBottom - waistY) / (height / 2)
		leftX := cx - widthAtY/2
		rightX := cx + widthAtY/2

		vertices := []ebiten.Vertex{
			{DstX: float32(cx), DstY: float32(waistY), SrcX: 0, SrcY: 0, ColorR: 210.0 / 255, ColorG: 180.0 / 255, ColorB: 140.0 / 255, ColorA: 200.0 / 255},
			{DstX: float32(leftX), DstY: float32(fillYBottom), SrcX: 0, SrcY: 0, ColorR: 210.0 / 255, ColorG: 180.0 / 255, ColorB: 140.0 / 255, ColorA: 200.0 / 255},
			{DstX: float32(rightX), DstY: float32(fillYBottom), SrcX: 0, SrcY: 0, ColorR: 210.0 / 255, ColorG: 180.0 / 255, ColorB: 140.0 / 255, ColorA: 200.0 / 255},
		}
		indices := []uint16{0, 1, 2}
		screen.DrawTriangles(vertices, indices, whiteTex, nil)
	}
