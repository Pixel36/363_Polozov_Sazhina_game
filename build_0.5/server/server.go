...
const (
	...
	tileSize    = 32
	maxSpeed    = 8.0
	maxPlayers  = 10 // изменено с 20 на 10
...
var (
	...
	// Пошаговый режим
	playersOrder  []string
	currentTurn   int // индекс в playersOrder
...
func main() {
	...
	fmt.Println("=== RPG Сервер (пошаговый) ===")
...
func wsHandler(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("📥 Игрок подключился: %s (%s) ID: %s на позиции %.0f,%.0f", name, race, id, x, y)
...
func handleTurnAction(playerID string, msg map[string]any) {
	...
	// Проверка таймаута (на случай, если таймер ещё не сработал, но время вышло)
	turnMu.Lock()
	...
	// После успешного действия переключаем ход
	turnMu.Lock()
	nextTurn()
	turnMu.Unlock()
...
func broadcastToAll() {
	...
	if len(playersOrder) > 0 {
		...
		msg["turn_time_left"] = (turnTimeout - time.Since(turnStartTime)).Seconds()
	}
	turnMu.RUnlock()
