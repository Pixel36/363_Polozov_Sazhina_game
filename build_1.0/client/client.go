...
type Player struct {
	...
	AttackAnimTargetID string
	AttackAnimType     string
	AttackAnimProgress float64
}
...
func (g *Game) handleQuitConfirm() {
	...
	settingsRect := image.Rect(dx+(dw-btnW)/2, btnY2, dx+(dw-btnW)/2+btnW, btnY2+btnH)
...
func (g *Game) drawCharacterMenu(screen *ebiten.Image) {
	...
	inputRect := image.Rect(400, 140, 900, 240)
	...
	text.Draw(screen, displayName, charEditorFont, int(float64(inputRect.Min.X))+10, int(float64(inputRect.Min.Y))+70, color.Black)

	raceLabel := "Раса:"
	text.Draw(screen, raceLabel, charEditorFont, 200, 320, color.Black)

	humanBtn := image.Rect(400, 280, 650, 380)
	...
	catBtn := image.Rect(670, 280, 920, 380)
	...
	weaponLabel := "Оружие:"
	text.Draw(screen, weaponLabel, charEditorFont, 200, 440, color.Black)

	swordBtn := image.Rect(400, 400, 650, 500)
	...
	g.drawSwordScaled(screen, swordCenterX, swordCenterY, 0, 2.0, nil)

	spearBtn := image.Rect(670, 400, 920, 500)
	...
	g.drawSpearScaled(screen, spearCenterX, spearCenterY, 0, 2.0, nil)

	colorLabel := "Цвет:"
	text.Draw(screen, colorLabel, charEditorFont, 200, 640, color.Black)
	...
	for i, c := range g.charColors {
		...
		if i == g.charSelectedColor {
			ebitenutil.DrawRect(screen, float64(x-3), float64(y-3), float64(sw+6), float64(sh+6), color.White)
		}
		if g.colorTaken[i] {
	...
	if g.charPreviewImg != nil {
		...
		centerX := 1000 + float64(tileSize)*2.5
		centerY := 300 + float64(tileSize)*2.5

		if g.charRace == "cat" && g.charSelectedColor >= 0 {
			col := g.charColors[g.charSelectedColor]
			netCol := NetColor{R: col.R, G: col.G, B: col.B, A: col.A}
			g.drawCatEarsScaled(screen, centerX, centerY, netCol, 5.0)
		}

		if g.charWeapon == "sword" {
			weaponX := centerX + 80
			weaponY := centerY + 20
			g.drawSwordScaled(screen, weaponX, weaponY, math.Pi/4, 2.0, nil)
		} else {
			weaponX := centerX + 80
			weaponY := centerY + 20
			g.drawSpearScaled(screen, weaponX, weaponY, math.Pi/4, 2.0, nil)
		}
	}

	connectBtn := image.Rect(screenW/2+100, 800, screenW/2+500, 900)
	...
	if g.charError != "" {
		text.Draw(screen, "Ошибка: "+g.charError, charEditorFont, 200, 950, color.RGBA{255, 0, 0, 255})
	}
}
...
func (g *Game) drawGame(screen *ebiten.Image) {
	...
	if meCopy != nil {
		hpBarWidth := 200.0
		hpBarHeight := 20.0
		hpX := screenW - hpBarWidth - 20
		hpY := 20.0

		vector.DrawFilledRect(screen, float32(hpX), float32(hpY), float32(hpBarWidth), float32(hpBarHeight), color.RGBA{50, 0, 0, 200}, false)

		hpPercent := float64(meCopy.HP) / 10.0
		if hpPercent < 0 {
			hpPercent = 0
		}
		fillWidth := hpBarWidth * hpPercent
		vector.DrawFilledRect(screen, float32(hpX), float32(hpY), float32(fillWidth), float32(hpBarHeight), color.RGBA{255, 0, 0, 255}, false)

		hpText := fmt.Sprintf("HP: %d / 10", meCopy.HP)
		text.Draw(screen, hpText, g.fontFace, int(hpX+10), int(hpY+hpBarHeight-4), color.White)
	}
...
func (g *Game) drawQuitConfirm(screen *ebiten.Image) {
	...
	btnY2 := btnY1 + btnH + spacing
	settingsRect := image.Rect(g.quitConfirmRects.bg.Min.X+(g.quitConfirmRects.bg.Dx()-btnW)/2, btnY2, g.quitConfirmRects.bg.Min.X+(g.quitConfirmRects.bg.Dx()+btnW)/2, btnY2+btnH)
